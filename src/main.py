...
def render(
    screen, p, scale_height, distance, color_map, height_map, debug_rendering=False
):
    ...
    while z < distance:
        pleft = pygame.Vector2(
            (-cosphi * z - sinphi * z) - p.pos.y, (sinphi * z - cosphi * z) - p.pos.x
        )
        ...
        for i in range(0, screen_width):
            ...
            if height_on_screen < ybuffer[i]:
                ...
                pygame.gfxdraw.vline(
                    screen, i, int(height_on_screen), int(ybuffer[i]), color
                )
                if debug_rendering:
                    pygame.display.flip()
